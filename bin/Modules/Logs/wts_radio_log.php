...
function showRadioLog($config, $dateSelect, $counter, $logType, $radioLogID, $filters, $isApprove = false) {
    $mysqli = $config->mysqli;
    ...
    if (!$isApprove) {
        if ($logType == "PERM") {
            ...
            $myq = "SELECT R.REFNUM, 
                    CASE WHEN T.DESCR = '' THEN
                        CONCAT('Car #', INV.SERIAL_NUM)
                        ELSE T.DESCR
                    END AS 'itemType', 
                    R.GPNUM 'gpID', CONCAT_WS(', ',SEC.LNAME,SEC.FNAME) 'DEPUTYID', 
            ...
            if ($showAll)
                ...
                $showAllQ = "R.CHECKEDOUT=1";
            $myq = "SELECT R.REFNUM, CASE WHEN T.DESCR = '' THEN
                        CONCAT('Car #', INV.SERIAL_NUM)
                        ELSE T.DESCR
                    END AS 'itemType',  R.GPNUM 'gpID', CONCAT_WS(', ',SEC.LNAME,SEC.FNAME) 'DEPUTYID', 
                    INV.OTHER_SN, R.RADIO_CALLNUM, R.TYPE 'checkOutType', R.CHECKEDOUT 'isCheckedOut',
    ...
    if ($config->adminLvl >= 0) {
        if ($logType == "LOANER") {
            ...
            if (!$isApprove) {
                ...
                $theTable[$x][$y] = "Approve";
                $y++;
            }
            $theTable[$x][$y] = "Type";
            $y++;
            $theTable[$x][$y] = "Serial Number";
            $y++;
            $theTable[$x][$y] = "Details";
            $y++;
            $theTable[$x][$y] = "Deputy";
            $y++;
            $theTable[$x][$y] = "Radio Call#";
            $y++;
            $theTable[$x][$y] = "Type";
            $y++;
            $theTable[$x][$y] = "OUT_Time";
            $y++;
            $theTable[$x][$y] = "OUT_By";
            $y++;
            $theTable[$x][$y] = "In_Time";
            $y++;
            $theTable[$x][$y] = "In_By";
            $y++;
            $theTable[$x][$y] = "Status";
        ...
        while ($row = $result->fetch_assoc()) {
            if ($logType == "LOANER") {
                if (strcmp($row['checkIn'], "00/00/00 0000") == 0 || $showAll || (strcmp($row['SUP_TIME'], "00/00/00 0000") == 0 && $isApprove)) {
                    ...
                    $y = 1;
                    $theTable[$x][$y] = $row['itemType'] . '<input type="hidden" name="itemLogType' . $x . '" value="' . $row['itemType'] . '" />';
                    $y++;
                    $theTable[$x][$y] = $row['OTHER_SN'];
                    $y++;
                    $theTable[$x][$y] = $row['itemDesc'];
                    $y++;
                    $deputyName = $row['DEPUTYID'];
                    if ($deputyName == "SYSTEM, USER") {
                        $theTable[$x][$y] = $row['COMMENTS'];
                        $y++;
                    } else {
                        if ($config->adminLvl >= 25)
                    ...
                    $theTable[$x][$y] = $row['RADIO_CALLNUM'];
                    $y++;
                    $theTable[$x][$y] = $row['checkOutType'];
                    $y++;
                    $theTable[$x][$y] = $row['checkOut'];
                    $y++;
                    ...
                    if ($row['isCheckedOut'] == 1) {
                        $theTable[$x][$y] = '<input type="submit" name="checkInRadio' . $x . '" value="Check Back In" /><br/>
                            <input type="submit" name="exchangeBtn' . $x . '" value="Exchange" />';
                        $y++;
                    } else {
                        $theTable[$x][$y] = "Checked In";
                        $y++;
                    }
                    if ($config->adminLvl >= 25 && false) {
                ...
                if ($config->adminLvl >= 25) {
                    $theTable[$x][$y] = 'Ref# ' . $row['REFNUM'] . '<input type="submit" value="Edit/View" name="radioLogEditBtn' . $counter . '" />
                        <input type="hidden" name="radioLogID' . $counter . '" value="' . $row['REFNUM'] . '" />';
                    $y++;
                } else {
                ...
                $theTable[$x][$y] = $row['itemType'] . '<input type="hidden" name="itemLogType' . $counter . '" value="' . $row['itemType'] . '" />';
                $y++;
                $theTable[$x][$y] = $row['OTHER_SN'];
                $y++;
                if ($config->adminLvl >= 25)
                ...
                if ($row['isCheckedOut'] == 1) {
                    if ($config->adminLvl > 25) {
                        $theTable[$x][$y] = '<input type="submit" name="checkInRadio' . $counter . '" value="Check Back In" />';
                        $y++;
                    } else {
                        $theTable[$x][$y] = "Checked Out";
                        $y++;
                    }
                } else {
                    $theTable[$x][$y] = "Checked In";
                ...
                if ($row['isCheckedOut'] == 1) {
                    if ($config->adminLvl >= 0) {
                        ...
                        $theTable[$x][$y] .= '<input type="submit" name="exchangeBtn' . $counter . '" value="Exchange" />';
                        $y++;
                    }
                    else {
                        $theTable[$x][$y] = "Checked Out";
...
function checkOutItem($config, $deputyID, $radioCallNum, $itemID, $itemTypeID, $checkOutType, $isReserve, $groupID, $divID, $noLog = false, $comments = '') {
    $mysqli = $config->mysqli;
    ...
    if ($verifyCount['isAvailable'] == "1") {
        $myq = "INSERT INTO WTS_RADIOLOG ( REFNUM ,`DEPUTYID`,`RADIO_CALLNUM` , CHECKEDOUT, RADIOID, TYPE,
                `AUDIT_OUT_ID` ,`AUDIT_OUT_TS` ,`AUDIT_OUT_IP`, IS_RESERVE, GPNUM, DIVISIONID, ITEM_TYPE_ID, COMMENTS) 
            VALUES ( NULL , '" . $deputyID . "', '" . $radioCallNum . "', '1', '" . $itemID . "',
                '" . $checkOutType . "', '" . $_SESSION['userIDnum'] . "', NOW(), 
                INET_ATON('" . $_SERVER['REMOTE_ADDR'] . "'), '" . $isReserve . "', '" . $groupID . "', '" . $divID . "', '" . $itemTypeID . "', '" . $comments . "' );";
        $result = $mysqli->query($myq);
        if (!SQLerrorCatch($mysqli, $result, $myq)) {
