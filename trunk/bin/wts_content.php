 ...
 */
   ...
   if (isset($_POST['saveBtn'])) { //saveBtn created in resultTable()
    ...
    while($finfo = mysqli_fetch_field($result)) {
       ...
       //idea is to use the above to write the update statement below
        for ($i=0; $i < $numOfCols; $i++)
            $newValue[$i] = $_POST["$i"]; //put all form values into array
            
    //match form values with fields to be updated
    $updateQuery="UPDATE REQUEST 
        SET TIMETYPEID='$newValue[2]', USEDATE='$newValue[4]', HOURS='$newValue[5]',
        NOTE='$newValue[6]', APPROVE='$newValue[7]', REASON='$newValue[8]',
        AUDITID='${_SESSION['userName']}'
